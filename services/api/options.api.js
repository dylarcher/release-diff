...
export class SettingsManager {
  ...
  setupEventListeners() {
    const { saveButton, themeToggle, testBtn, testJiraBtn, testGitLabBtn } =
      this.elements;
    ...
    if (themeToggle) {
      themeToggle.addEventListener(
        "change",
        this.eventHandlers.get("themeChange")
      );
    }
    ...
    if (testGitLabBtn) {
      testGitLabBtn.addEventListener(
        "click",
        this.eventHandlers.get("testGitLab")
      );
  ...
  async handleSave() {
    ...
    try {
      ...
      displayStatusWithAutoHide(
        statusDiv,
        getMessage(USER_MESSAGES.SETTINGS_SAVED_SUCCESSFULLY),
      ...
      displayStatusWithAutoHide(
        statusDiv,
        getMessage(USER_MESSAGES.ERROR_SAVING_SETTINGS),
        STATUS_TYPES.ERROR
      );
    ...
    try {
      ...
      const resultHandler = {
        true: () => {
          ...
          if (response.data) {
            console.info(
              `${action} ${CONSOLE_MESSAGES.RESPONSE_DATA}`,
              response.data
            );
          }
        },
  ...
  showFallbackOption(action) {
    const { statusDiv } = this.elements;
    const apiType = action.includes("JIRA")
      ? "jira"
      : action.includes("GITLAB")
        ? "gitlab"
        : "both";
